...
for y in years:
    for m in range(len(month)):
        ...
        if month[m] == "03" and y == "2022":
            isFuture = True
        if month[m] == "05" and y == "2023":
            break

nbr_adults = [nbr for nbr in range(1, 5, 3)]
nbr_children = [nbr for nbr in range(0, 5, 2)]
nbr_rooms = [nbr for nbr in range(2, 5, 3)]
city = "paris"
...
for start, end in zip(start_date, end_date):
    ...
    pool = ThreadPool(15)
    pool.map(lambda scrap: scrap.main(), scraper_list)
    print("je suis passe a autre chose")
